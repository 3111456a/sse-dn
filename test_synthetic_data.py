...
def load_all_synth_data(extended=False):
    # 请将这里的路径替换为你刚才运行 inference 脚本生成的实际路径
    # 例如：r'D:\see-dn\data\pred_denoising_test_data'
    my_pred_file = r'D:\see-dn\data\pred_denoising_test_data'

    # 我们只需要加载这一个文件
    X, y, catalogue, pred_graph = _load_synth_data(my_pred_file)

    # 依然返回一个列表，里面只有你这一个深度学习模型的预测结果
    return X, y, catalogue, [pred_graph]
...
if __name__ == '__main__':
    ...
    # 1. 把这一行的注释符号 '#' 删掉，让它现场计算传统滤波模型的预测结果
    # 将 save=True 改为 save=False，直接在内存中使用，不存硬盘
    mean_median_predictions = compute_mean_median_predictions(X, kernel_values, extended=extended_dataset, save=False)

    # 2. 把下面这两行试图读取本地文件的代码给注释掉（或者直接删掉）
